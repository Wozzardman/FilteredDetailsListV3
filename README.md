...
- **Advanced operators**: Contains, equals, greater than, in/not in, etc.
...
Table(
    {ColName: "name", ColDisplayName: "Name", ColWidth: 200, ColFilterable: true},
    {ColName: "status", ColDisplayName: "Status", ColWidth: 120, ColFilterable: true}
)
```
...
- `EnableFiltering` - True when filtering functionality should be enabled.
- `AppliedFilters` - JSON string of filters to apply on component load.
- `FilterEventName` - Output Event when a filter is applied/changed. Value will be `FilterChanged`.
- `FilterEventColumn` - Output Event column field name when a filter is triggered.
- `FilterEventValues` - Output Event JSON string containing the filter values that were applied.
- `AllFilters` - Output property containing JSON string of all currently active filters.

### Column Filter Properties

When defining columns, you can also specify:
- `ColFilterable` - True when the column should be filterable (shows filter icon in header).
- `ColFilterType` - Override the automatic filter type detection. Values: `text`, `number`, `date`, `boolean`, `choice`.
