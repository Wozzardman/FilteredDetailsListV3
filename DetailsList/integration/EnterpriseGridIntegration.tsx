...
import { EnterpriseTestDataGenerator, createTestDataset, createTestColumns } from '../testing/EnterpriseTestDataGenerator';
...
export const EnterpriseGridIntegration: React.FC<EnterpriseGridIntegrationProps> = (props) => {
    ...
    const { items, columns } = useMemo(() => {
        ...
        try {
            ...
            const memoryEstimate = testDataGenerator.current.estimateMemoryUsage(
                testItems.length, 
                testColumns.length
            );
    ...
    const makeGridDecision = useCallback((itemCount: number, columnCount: number): GridDecision => {
        ...
        return (
            <div className="enterprise-grid-loading">
                ...
                <div className="loading-text">
                    Generating enterprise test data...
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="grid-info">
                <div className="grid-stats">
                    <span className="stat">
                        📊 {loadMetrics.recordCount.toLocaleString()} records
                    </span>
                    <span className="stat">
                        📋 {loadMetrics.columnCount} columns
                    </span>
                    <span className="stat">
                        ⚡ {gridDecision.useVirtualization ? 'Virtualized' : 'Standard'}
                    </span>
                    <span className="stat">
                        🎯 {gridDecision.estimatedPerformance}
                    </span>
                </div>
    ...
    const renderGrid = () => {
        ...
        if (gridDecision.useVirtualization) {
            console.log('🚀 Rendering HighPerformanceVirtualGrid');
            return (
                <HighPerformanceVirtualGrid
                    items={items}
                    columns={columns}
                    height={600}
                    width={800}
            ...
            return (
                <div className="standard-grid-fallback">
                    <div className="grid-message">
                        Standard grid rendering {items.length.toLocaleString()} records
                    </div>
                    <div className="grid-stats">
                        Performance mode: {gridDecision.estimatedPerformance}
                    </div>
                </div>
            );
        }
...
export type { 
    EnterpriseGridIntegrationProps, 
    PerformanceAlert, 
    DataLoadMetrics, 
    GridDecision 
};
